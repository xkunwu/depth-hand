...
class ops_abc(object):
    ...
    @classmethod
    def raw_to_pca(cls, points3, resce=np.array([1, 0, 0, 0])):
        cube = iso_cube()
        cube.load(resce)
    ...
    @classmethod
    def pca_to_raw(cls, points3, resce=np.array([1, 0, 0, 0])):
        cube = iso_cube()
        cube.load(resce)
    ...
    @classmethod
    def raw_to_local(cls, points3, resce=np.array([1, 0, 0, 0])):
        cube = iso_cube()
        cube.load(resce)
        # return cube.transform_center_shrink(points3)
        return cube.transform_to_center(points3)
    ...
    @classmethod
    def local_to_raw(cls, points3, resce=np.array([1, 0, 0, 0])):
        cube = iso_cube()
        cube.load(resce)
        # return cube.transform_expand_move(points3)
        return cube.transform_add_center(points3)
    ...
    @classmethod
    def d2z_to_raw(cls, p2z, caminfo, resce=np.array([1, 0, 0])):
    ...
    @classmethod
    def raw_to_2dz(cls, points3, caminfo, resce=np.array([1, 0, 0])):
    ...
    @classmethod
    def raw_to_2d(cls, points3, caminfo, resce=np.array([1, 0, 0])):
        pose2d, _ = cls.raw_to_2dz(points3, caminfo, resce)
        # print(points3)
        # print(pose2d)
        return pose2d

    @classmethod
    def raw_to_heatmap2(cls, pose_raw, cube, caminfo):
    ...
    @classmethod
    def raw_to_udir2(cls, image_crop, pose_raw, cube, caminfo):
    ...
    @classmethod
    def raw_to_offset(cls, image_crop, pose_raw, cube, caminfo):
    ...
    @classmethod
    def offset_to_raw(
        cls, hmap2, olmap, uomap, image_crop,
            cube, caminfo, nn=5):
    ...
    @classmethod
    def estimate_z(cls, l3, l2, focal):
    ...
    @classmethod
    def recover_from_rect(cls, rect, region_size, caminfo):
        z_cen = cls.estimate_z(region_size, rect.sidelen / 2, caminfo.focal[0])
        centre = cls.d2z_to_raw(
    ...
    @classmethod
    def img_to_raw(cls, img, caminfo, crop_lim=None):
        ...
        points3 = cls.d2z_to_raw(indz, caminfo)
    ...
    @classmethod
    def normalize_depth(cls, img, caminfo):
    ...
    @classmethod
    def resize_normalize(cls, img, caminfo):
        """ rescale to fixed cropping size """
        img_rescale = cv2resize(
            img, (caminfo.crop_size, caminfo.crop_size))
        img_rescale = cls.normalize_depth(img_rescale, caminfo)
        return img_rescale

    @classmethod
    def frame_size_localizer(cls, img, caminfo):
    ...
    @classmethod
    def getbm(cls, base_z, caminfo, base_margin=20):
    ...
    @classmethod
    def clip_image_border(cls, rect, caminfo):
    ...
    def generate_anchors_2d(cls, img, pose_raw, anchor_num, caminfo):
        ...
        rect = cube.proj_to_rect(
            caminfo.region_size, cls.raw_to_2d, caminfo)
    ...
    @classmethod
    def trunc_belief(cls, pcnt):
        pcnt_r = np.copy(pcnt)
        befs = []
        for spi in range(3):
            pcnt_r = np.rollaxis(pcnt_r, -1)
            befs.append(cls.direc_belief(pcnt_r))
        return np.stack(befs, axis=3)

    @classmethod
    def prop_edt2(cls, image_crop, pose_raw, cube, caminfo, roll=0):
    ...
    @classmethod
    def prop_ov3edt2(cls, ortho3_crop, pose_raw, cube, caminfo, roll=0):
        crop_size = caminfo.crop_size
        images = ortho3_crop.reshape((crop_size, crop_size, 3))
        edt_l = []
        for dd in range(3):
            edt_l.append(cls.prop_edt2(
    ...
    @classmethod
    def to_pcnt3(cls, img, cube, caminfo):
    ...
    @classmethod
    def raw_to_vxoff(cls, vxcnt, pose_raw, cube, caminfo):
        """ offset map from voxel center to each joint
        """
        step = caminfo.hmap_size
        offset = cls.raw_to_vxoff_flat(vxcnt, pose_raw, cube, caminfo)
        void_id = cls._void_id(vxcnt, step)
        offset[void_id] = 0
        return offset.reshape(step, step, step, -1)

    @classmethod
    def raw_to_vxudir(cls, vxcnt, pose_raw, cube, caminfo):
        """ offset map from voxel center to each joint
        """
        from numpy import linalg
        step = caminfo.hmap_size
    ...
    @classmethod
    def to_ortho3(cls, img, cube, caminfo, sort=False):
        points3_pick = cube.pick(cls.img_to_raw(img, caminfo))
    ...
    @classmethod
    def get_rect3(cls, cube, caminfo):
        """ return a rectangle with margin that 3d points
            NOTE: there is still a perspective problem
        """
        cen = cls.raw_to_2d(cube.cen.reshape(1, -1), caminfo).flatten()
        rect = iso_rect(
            cen - cube.sidelen,
            cube.get_sidelen()
        )
        rect = cls.clip_image_border(rect, caminfo)
        return rect

    @classmethod
    def to_clean(cls, img, cube, caminfo, sort=False):
        points3_pick = cube.pick(cls.img_to_raw(img, caminfo))
    ...
    @classmethod
    def get_rect2(cls, cube, caminfo):
        rect = cube.proj_to_rect(
            caminfo.region_size, cls.raw_to_2d, caminfo)
        rect = cls.clip_image_border(rect, caminfo)
        return rect
    ...
    @classmethod
    def to_crop2(cls, img, cube, caminfo):
        rect = cls.get_rect2(cube, caminfo)
        # import matplotlib.pyplot as mpplot
        # mpplot.imshow(img, cmap=mpplot.cm.bone_r)
        # rect.show_dims()
        # rect.draw(ax)
        ...
        img_crop2 = cls.resize_normalize(img_crop, caminfo)
        return img_crop2
