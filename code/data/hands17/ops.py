...
def get3d(thedata, pose_mat, rescen=None):
    ...
    pose3d = (pose2d - thedata.centre) / thedata.focal * pose_z
    return np.hstack((pose3d, pose_z))


def get2d(thedata, points3):
    """ project 3D point onto image plane using camera info
        Args:
            points3: nx3 array
    """
    pose_z = np.array(points3[:, 2]).reshape(-1, 1)
    return points3[:, 0:2] / pose_z * thedata.focal + thedata.centre


def getbm(thedata, base_z, base_margin=20):
    """ return margin (x, y) accroding to projective-z of MMCP.
        Args:
            base_z: base z-value in mm
            base_margin: base margin in mm
    """
    marg = np.tile(base_margin, (2, 1)) * thedata.focal / base_z
    m = max(marg)
    return m


def get_rect(thedata, pose2d, bm):
    ...
    ctl = cen - ptp_m / 2
    cbr = ctl + ptp_m
    obm = np.min([ctl, thedata.image_size - cbr])
    if 0 > obm:
        # print(ctl, thedata.image_size - cbr, obm, ptp_m)
        ptp_m = ptp_m + 2 * obm
    ctl = cen - ptp_m / 2
